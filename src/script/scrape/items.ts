...
// Types pour les données des items TFT
interface ItemStats {
  tier: number;
  score: number;
  nb_games: number;
...
interface TFTItemsData {
  [itemKey: string]: ItemStats;
}
...
class TFTItemsScraper {
  ...
  private outputDir = './data/tft/items';
  ...
  private async downloadItemImage(itemApiName: string): Promise<string | null> {
    try {
      ...
      const response = await axios.get(imageUrl, {
        ...
        timeout: 10000,
        headers: {
          'User-Agent': 'TFT-Items-Scraper/1.0',
        }
      });
   ...
   */
  private validateData(data: any): data is TFTItemsData {
  ...
  async fetchItemsData(): Promise<ScrapingResult> {
    ...
    try {
      console.log('🔄 Récupération des données des items TFT...');
      console.log(`📡 URL: ${this.baseUrl}`);
      
      const response = await axios.get<TFTItemsData>(this.baseUrl, {
        timeout: 15000,
        headers: {
          'User-Agent': 'TFT-Items-Scraper/1.0',
  ...
  async saveData(data: ItemsCollection, filename?: string): Promise<void> {
    try {
      const fileName = filename || `tft_items_stats_${Date.now()}.json`;
    ...
    try {
      ...
      for (const [tier, items] of Object.entries(tierData)) {
        if (items.length > 0) {
          ...
          await fs.writeFile(
            path.join(this.outputDir, `tft_items_tier_${tier}_${timestamp}.json`),
    ...
    try {
      ...
      for (const [type, items] of Object.entries(typeData)) {
        if (items.length > 0) {
          ...
          await fs.writeFile(
            path.join(this.outputDir, `tft_items_type_${type}_${timestamp}.json`),
            JSON.stringify(items, null, 2),
            'utf-8'
          );
        }
  ...
  async generateReport(data: ItemsCollection, stats: AnalysisStats): Promise<void> {
    try {
      let report = '# TFT Set 14 - Items Statistics Report\n\n';
      ...
      const reportPath = path.join(this.outputDir, `items_report_${Date.now()}.md`);
  ...
  async generateItemsForApp(data: ItemsCollection): Promise<void> {
    try {
      const items = Object.values(data).map(item => ({
        ...
        name: this.cleanItemName(item.apiName),
        desc: "",
        shortDesc: "",
        fromDesc: "",
      ...
      const appItemsPath = path.join('./src/data', 'items_stats_generated.ts');
      
      let content = `// Généré automatiquement le ${new Date().toLocaleString('fr-FR')}\n`;
      content += `import { ItemStats } from '../types';\n\n`;
      content += `export const generatedItemsStats: ItemStats[] = [\n`;
  ...
  async saveScrapingLog(result: ScrapingResult): Promise<void> {
    try {
      const logPath = path.join(this.outputDir, 'items_scraping_log.json');
  ...
  async scrape(options: { 
    ...
    console.log('🚀 Démarrage du scraping TFT Items...\n');
...
async function runItemsScraper() {
  const scraper = new TFTItemsScraper();
  ...
  if (continuous) {
    console.log('🔄 Mode continu activé (vérification toutes les 30 minutes)');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runItemsScraper().catch(console.error);
}